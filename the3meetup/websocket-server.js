...
class WebSocketManager {
  ...
  initialize(port = 3001) {
    console.log(`[${this.id}] 开始初始化HTTP/WS服务器，端口: ${port}`);
    ...
    server.listen(port, () => {
      ...
      console.log(`✅ 协议: http/ws`);
      console.log(`✅ HTTP API: http://${host}:${port}`);
      console.log(`✅ WebSocket URL: ws://${host}:${port}`);
      if (process.env.WS_DOMAIN) {
        console.log(`✅ 域名访问: http://${process.env.WS_DOMAIN}:${port}`);
        console.log(`✅ WebSocket 域名: ws://${process.env.WS_DOMAIN}:${port}`);
      }
    });
